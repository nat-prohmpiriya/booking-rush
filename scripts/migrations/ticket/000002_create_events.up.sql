...
-- Ticket DB: Events table (NO FK to tenants/users - cross-database references)

CREATE TYPE event_status AS ENUM ('draft', 'published', 'cancelled', 'completed');

CREATE TABLE IF NOT EXISTS events (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL,      -- Reference only, NO FK (cross-database)
    organizer_id UUID NOT NULL,   -- Reference only, NO FK (cross-database to auth_db.users)
